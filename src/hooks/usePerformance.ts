...
const usePerformance = (options: UsePerformanceOptions = {}) => {
  ...
  const startPerformanceTimer = useCallback((label: string) => {
    if (enableMetrics) {
  ...
  const endPerformanceTimer = useCallback((label: string, updateMetric?: keyof PerformanceMetrics) => {
  ...
  const getOptimizationRecommendations = useCallback((content: string) => {
    const analysis = analyzeContent(content);
    const recommendations: string[] = [];

    if (analysis.isLarge) {
      recommendations.push('Consider enabling virtual scrolling for large files');
    ...
    if (analysis.size > 5 * 1024 * 1024) { // 5MB
      recommendations.push('File is very large - consider splitting into smaller files');
