...
const EditorToolbar: React.FC<EditorToolbarProps> = ({
  ...
  const Separator = () => (
    <div
      className={`w-px h-6 ${theme === 'dark' ? 'bg-gray-700' : 'bg-gray-300'}`}
    />
  );
  ...
  return (
    ...
    >
      {/* File operations */}
      <ToolbarButton onClick={onOpen} title="Open file (Ctrl+O)">
        Open
      </ToolbarButton>
      <ToolbarButton onClick={onSave} title="Save file (Ctrl+S)">
        Save
      </ToolbarButton>
      ...
      >
        <div className="flex items-center">
          Validate
          {!isValid && <div className="ml-1 w-2 h-2 bg-red-500 rounded-full" />}
        </div>
      </ToolbarButton>
      ...
      {/* Utility operations */}
      <ToolbarButton onClick={onSearch} title="Find and Replace (Ctrl+F)">
        Search
      </ToolbarButton>
